...
function CarDetailPage({user}) {
  ...
  useEffect(() => {
    ...
    const favoriteCheckReq = axios.get(
  ...
  const handleFavorite = async (e) => {
    ...
    try {
      ...
      if (e.response?.data?.error === "ALREADY_FAVORITE") {
        alert("이미 찜한 차량입니다!");
      } else {
        alert('찜목록에 추가를 실패했습니다. 나중에 다시 시도해주세요.')
      }
    }
  }
              ...
              <div className="side-price-card">
                <div className="side-price-top">
                  ...
                  <div className="side-price-main">
                    ...
                    <button className={`wish-btn ${isFavorite ? "active" : ""}`} onClick={handleFavorite}>
                      <span className="wish-icon">♡</span>
                      <span className="wish-text">찜하기</span>
                    </button>
                  </div>
